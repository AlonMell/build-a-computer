...
class HackAssembler:
    ...
    def __handle_label(self, line: str, line_number: int) -> None:
        ...
        if label not in self.__symbolTable:
            self.__symbolTable[label] = line_number
    ...
    def __preprocess(self, asm_file: TextIO) -> list[str]:
        ...
        for line in asm_file:
            if self.__skip_line(line):
                continue
            elif self.__is_starts_with(line, self.__LABEL):
    ...
    def __handle_A_instruction(
        ...
        if a_instr.isdigit():
            return bin(int(a_instr))[2:].zfill(self.__MACHINE_WORD)
        elif a_instr not in self.__symbolTable:
            self.__symbolTable[a_instr] = var_addr[0]
            var_addr[0] += 1

        return bin(self.__symbolTable[a_instr])[2:].zfill(self.__MACHINE_WORD)
    ...
    def __parse(self, line: str) -> str:
        ...
        if is_dest_find:
            dest = self.__cTable["dest"][line[:dest_idx]]
        if is_jump_find:
            jump = self.__cTable["jump"][line[jump_idx+1:]]

        if is_dest_find and is_jump_find:
            comp = self.__cTable["comp"][line[dest_idx+1:jump_idx]]
        elif not is_dest_find and is_jump_find:
            comp = self.__cTable["comp"][line[:jump_idx]]
        else:
            comp = self.__cTable["comp"][line[dest_idx+1:]]
    ...
    def Compile(self, in_file_path: str, out_file_path: str = "") -> None:
        if out_file_path == "":
            file_base_name, _ = os.path.splitext(os.path.basename(in_file_path))
...
def parse_arguments():
    ...
    parser.add_argument("-o", "--outFile", type=str, help="Path to the output .hack file", default="")
